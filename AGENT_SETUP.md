...
| A live interactive session never shows a Close button | Check `pid` in `/api/sessions` for that session — it's `null` when there's no registry file for it (rare: only happens if `sessions/<pid>.json` was already cleaned up while the process is somehow still live). Not fixable client-side; there is no other source for a pid. | Confirm via `Get-Process`/`tasklist` whether the pid genuinely still exists. If it does but the registry file is gone, that's a Claude Code-side gap, not a ResumerAgent bug. |
| `/api/close` returns 500 with a taskkill error even though the session's process is confirmed gone afterward | `taskkill /T` fails its overall exit code if ANY descendant process already exited on its own before taskkill reached it — already handled on `master` (`lib/closeSession.mjs` re-checks the target pid specifically and only surfaces the error if the target itself is still running). If you see this again, the regression is almost certainly that re-check being removed or short-circuited. | Run `lib/closeSession.test.mjs`'s race-condition case before touching this file again; don't "fix" it by swallowing every taskkill error — a target that's genuinely still running after a failed close must still surface as a failure. |
| Close ended a session's whole terminal window, not just its `claude` process | Shouldn't happen — `taskkill /PID <pid> /T /F` targets only the session's own pid and its descendants, never its parent `cmd.exe`. If the window did close, check whether `pid` on that session actually pointed at the parent shell rather than the `claude`/`claude.exe` process itself (would mean a registry-read bug upstream, not this file). | Verify with `Get-CimInstance Win32_Process -Filter "ProcessId=<pid>"` that the pid in question really is `claude.exe`/`claude`, not `cmd.exe`, before assuming this file is at fault. |
