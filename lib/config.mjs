...
export function loadConfig(env = process.env) {
  ...
  // Undefined (not a default string) when unset — lib/terminalCommand.mjs
  // treats a falsy override as "use this platform's built-in default", so
  // leaving this out entirely is how that opt-in stays opt-in.
  const terminalCommand = merged.TERMINAL_COMMAND || undefined;

  const port = Number.parseInt(merged.PORT, 10) || 4317;

  return { sessionsRoot, resumeCommand, attachCommand, terminalCommand, port, cleanupPeriodDays };
}
