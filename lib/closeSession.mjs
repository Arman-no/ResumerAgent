...
export async function closeInteractiveSession(pid, overrides = {}) {
  ...
  return platform === 'win32' ? closeOnWindows(pid, overrides) : closeOnPosix(pid, overrides);
}

// POSIX has neither taskkill /T nor a PowerShell CIM lookup, but it needs
// fewer calls, not more: one `ps -eo pid=,ppid=,comm=` read answers all
// three questions the Windows path makes separate calls for — is this pid
// still a claude process, what is its parent, and what are its descendants.
async function closeOnPosix(pid, overrides) {
  const {
    psCommand = 'ps',
    psArgsPrefix = [],
    killCommand = 'kill',
  ...
  const table = await readProcessTable(psCommand, psArgsPrefix);
  const target = table.get(pid);
  if (!isClaudeEntry(target)) {
    throw new Error(`PID ${pid} is no longer a claude process — refusing to act on a possibly-reused PID`);
  }

  const parentPid = target.ppid;
  const tree = [pid, ...descendantsOf(pid, table)];

  try {
    await execFileAsync(killCommand, [...killArgsPrefix, '-9', ...tree.map(String)], {
      windowsHide: true,
      timeout: TIMEOUT_MS,
      env: envWithoutIdentity(),
    });
  } catch (err) {
    // Same race the Windows /T path hits: a helper can exit on its own
    // between reading the table and the kill reaching it. Only the target
    // itself decides whether this failed.
    const after = await readProcessTable(psCommand, psArgsPrefix);
    if (isClaudeEntry(after.get(pid))) throw err;
  }

  if (parentPid) {
    try {
      await resetPosixTerminal(parentPid, psCommand, psArgsPrefix, devDir);
    } catch (err) {
...
function isClaudeEntry(entry) {
  return Boolean(entry) && entry.comm.toLowerCase().includes('claude');
}

async function readProcessTable(command, argsPrefix) {
  const { stdout } = await execFileAsync(command, [...argsPrefix, '-eo', 'pid=,ppid=,comm='], {
    windowsHide: true,
    timeout: TIMEOUT_MS,
    env: envWithoutIdentity(),
  });
  ...
  for (const line of stdout.split('\n')) {
    ...
    if (row) table.set(Number(row[1]), { ppid: Number(row[2]), comm: row[3].trim() });
  }
  return table;
}
...
function descendantsOf(pid, table) {
  ...
  while (queue.length > 0) {
    ...
    for (const [childPid, entry] of table) {
      if (entry.ppid === current && childPid !== pid && !found.includes(childPid)) {
        found.push(childPid);
        queue.push(childPid);
      }
    }
...
// The same xterm private modes scripts/reset-terminal-modes.ps1 disables,
// minus the RIS full reset it ends with: on a POSIX terminal RIS also wipes
// scrollback the user still wants.
const POSIX_TERMINAL_RESET =
  '\x1b[?1000l\x1b[?1002l\x1b[?1003l\x1b[?1006l\x1b[?1015l\x1b[?2004l\x1b[?1049l\x1b[?25h';

async function resetPosixTerminal(parentPid, psCommand, psArgsPrefix, devDir) {
  const { stdout } = await execFileAsync(
    psCommand,
    [...psArgsPrefix, '-p', String(parentPid), '-o', 'tty='],
    { windowsHide: true, timeout: TIMEOUT_MS, env: envWithoutIdentity() }
  );
  const tty = stdout.trim();
  // "?" means no controlling terminal, so there is nothing to restore.
  if (!tty || tty.startsWith('?')) return;
  fs.appendFileSync(path.join(devDir, tty), POSIX_TERMINAL_RESET);
}

async function closeOnWindows(pid, overrides) {
  const {
