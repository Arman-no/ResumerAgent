...
export function mergeSessions(transcriptEntries, registryEntries, liveEntries, getPreview) {
  ...
  //
  // Skipping on the mere *presence* of a state key (rather than these two
  // known terminal values) was tried first and is wrong: a real, actively
  // running background job (PHOENIX-18579, confirmed 2026-09-14, mid-chat
  // with the user) was listed with `state: "blocked"` — genuinely alive,
  // not historical. "state key present at all" is not Claude Code's
  // terminal-record signal after all; only these two observed values are.
