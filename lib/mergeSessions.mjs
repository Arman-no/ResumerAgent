...
export function mergeSessions(transcriptEntries, registryEntries, liveEntries, getPreview) {
  ...
  // its own, with no live companion entry at all, once it reaches a
  // terminal state — confirmed live 2026-09-13 two ways: pausing a real
  // background job via the new /api/pause (`claude stop <id>`) left it
  // listed with `state: "done"`, and a real crashed job on this machine
  // was found listed with `state: "failed"`. Both lacked a `state` key
  // entirely while genuinely running (confirmed on the same live job
  // before it was paused) — a `state` key present at all is exactly
  // Claude Code's own signal for "this is a historical record, not a
  // currently-running one," which is why this checks for the key's
  // presence rather than enumerating known terminal values (done,
  // failed, and whatever else may exist that hasn't been observed here).
  // Skip such a record outright rather than only preferring a live
  // companion when both exist — a terminal-only entry (exactly what
  // pausing produces) would otherwise read as Boolean(liveEntry) ===
  // true below, and the dashboard would show a session this app itself
  // just stopped (or one that crashed) as still live, with Attach
  // offered for a process that no longer exists.
