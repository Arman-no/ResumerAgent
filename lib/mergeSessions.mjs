...
export function mergeSessions(transcriptEntries, registryEntries, liveEntries, getPreview) {
  ...
  // finished background-job record (state: "done", no status) alongside
  // the session's own fresh interactive attach. Prefer whichever entry
  // isn't done — that's the one actually describing what's live right now.
  const liveBySessionId = new Map();
  for (const e of live) {
    const existing = liveBySessionId.get(e.sessionId);
    if (!existing || (existing.state === 'done' && e.state !== 'done')) {
      liveBySessionId.set(e.sessionId, e);
    }
  }
