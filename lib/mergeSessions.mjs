...
export function mergeSessions(transcriptEntries, registryEntries, liveEntries, getPreview) {
  ...
  // An interactive session that parks a background job under it (Claude
  // Code's own registry field) drops out of `claude agents --json --all`
  // entirely once it has, even while genuinely alive — confirmed
  // 2026-09-14. parkedJobId matches the job's own `id`, resolved into
  // parentSessionId/childSessionId cross-references below, once both
  // sides of the pair are known. Kept off the session objects themselves
  // until resolved, rather than a transient field on each one.
  const parkedJobIdBySessionId = new Map();
  ...
  const merged = transcriptEntries.map((entry) => {
    ...
    if (registryEntry?.parkedJobId) parkedJobIdBySessionId.set(entry.sessionId, registryEntry.parkedJobId);
  ...
  for (const liveEntry of liveBySessionId.values()) {
    ...
    if (registryEntry?.parkedJobId) parkedJobIdBySessionId.set(liveEntry.sessionId, registryEntry.parkedJobId);
    const {
      preview: livePreview,
  ...
  // Single level only, by construction: parkedJobId is one scalar field,
  // not a chain, matching Claude Code's own data model — a parent is
  // never itself a child.
  for (const [sessionId, parkedJobId] of parkedJobIdBySessionId) {
    const parent = merged.find((s) => s.sessionId === sessionId);
    const child = merged.find((s) => s.id === parkedJobId);
    if (parent && child) {
      parent.childSessionId = child.sessionId;
