...
// name — fall back to the same "meaningful first line" heuristic the
// preview already computes, then the last path segment of cwd, then a
// short slice of the id. Never the full sessionId unsliced; that's not a
// name, it's an identifier.
function fallbackName(session, preview) {
...
export function mergeSessions(transcriptEntries, registryEntries, liveEntries, getPreview) {
  ...
  const merged = transcriptEntries.map((entry) => {
    ...
    return {
      name: registryEntry?.name ?? fallbackName(entry, preview),
  ...
  for (const liveEntry of live) {
    ...
    merged.push({
      name: liveEntry.name ?? registryEntry?.name ?? liveEntry.sessionId.slice(0, 8),
      ...
      // discovery window), so the caller falls back to the cwd/sessionId
      // encoding-based lookup for this one entry.
